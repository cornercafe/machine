...
void LCD_draw(int16_t col, int16_t row, int16_t width, int16_t height, int16_t color)
{
...
int getCenterTextRow(String text) {
  return (320 - ((text.length() + 1) * 10)) / 2;
}
void showQRTitle(String qr) {
  P_COL = getCenterTextRow(qr);
  P_ROW = 210;
  B_COLOR = WHITE;
  Display_string(qr);
}
void showQR(String command) {
  int x = command.indexOf("X");
  int y = command.indexOf("Y");
  int z = command.indexOf("D");
  String x_val = command.substring(x + 1, y);
  String y_val = command.substring(y + 1, z);
  String d_val = command.substring(z + 1);
  LCD_rect((x_val.toInt() * 3) + 62 , (y_val.toInt() * 3) + 5, 3, 3, d_val.toInt() == 0 ? WHITE : BLACK);
}
void displayBottomBar(String data) {
  F_SIZE = 2;
  LCD_rect(0, 220, 320, 20, BLACK);
  P_COL = 0;
  P_ROW = 223;
  B_COLOR = BLACK;
  F_COLOR = WHITE;
...
void displayTopBar(String data) {
  F_SIZE = 2;
  LCD_rect(0, 0, 320, 20, BLACK);
  P_COL = 0;
  P_ROW = 5;
  B_COLOR = BLACK;
  F_COLOR = WHITE;
...
void print_centered_str(String data) {
  B_COLOR = WHITE;
  F_COLOR = BLACK;
  P_COL = getCenterTextRow(data);
  Display_string(data);
}
void showWelcomeScreen() {
  ...
  F_SIZE = 2;
  P_ROW = 80;
  print_centered_str("TO CORNER CAFE");
  P_ROW = 120;
...
void showOrderFailScreen() {
  LCD_clear(WHITE);
  P_ROW = 140;
  print_centered_str("ORDER FAILED");
}
void showOrderRetryScreen() {
  LCD_clear(WHITE);
  P_ROW = 140;
  print_centered_str("ORDER RETRYING..");
}
void showTxnFailedScreen() {
  LCD_clear(WHITE);
  P_ROW = 140;
...
void showTxnSuccessScreen() {
  LCD_clear(WHITE);
  P_ROW = 120;
  print_centered_str("PLACE GLASS");
  P_ROW = 140;
  print_centered_str("PRESS DISPENCE BUTTON");
}
void showDispensingScreen() {
  LCD_clear(WHITE);
  P_ROW = 140;
  print_centered_str("DISPENSING...");
}
void showThankyouScreen() {
  LCD_clear(WHITE);
  P_ROW = 140;
  print_centered_str("Thankyou.");
}
